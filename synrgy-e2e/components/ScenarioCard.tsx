...
type Completion = { scenario_id: string; completed_at: string };
  ...
  return (
    <Link href={`/scenario/${scenario.id}`}>
      ...
      >
        <div className="mb-3 flex items-start gap-3">
          <div className="relative">
            <div
              ...
              style={{ backgroundColor: completion ? "#1D9E75" : scenario.color }}
            >
              {completion ? "✓" : scenario.icon}
            </div>
          </div>
        ...
        <ProgressBar
          percent={percent}
          color={completion ? "#1D9E75" : scenario.color}
          hasFails={!completion && failed > 0}
        />
        ...
        <div className="mt-2 flex items-center gap-3 text-xs text-gray-400">
          {completion ? (
            ...
            <>
              <span>{tested}/{totalChecks} tested</span>
              {passed > 0 && <span className="text-status-pass">{passed} passed</span>}
